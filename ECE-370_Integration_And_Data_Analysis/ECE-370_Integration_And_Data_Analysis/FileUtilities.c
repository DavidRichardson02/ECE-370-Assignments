...
char* find_name_from_path(const char* filePathName)
{
    ...
    int filePathLength = strlen(filePathName) + 1, fileExtensionLength = strlen(fileExtension) + 1, directoryPathLength = strlen(directoryPathName) + 1;
    int fileNameLength = filePathLength - (directoryPathLength + fileExtensionLength);
    char *fileName;
 ...
 */
void write_file_numeric_data(const char *filename, double *data, int countDataEntries)
{
    // Open the file for append and update
    FILE *file = fopen(filename, "a+");
    if (file == NULL)
    ...
    // Iterate through the array of strings
    for (int i = 0; i < countDataEntries; i++)
    {
...
char *create_directory(const char *filePathName, const char *directoryName)
{
    ...
    if (status == 0)
    {
        printf("\n\nDirectory created in 'directoryPathName', '%s'.", dataDirectoryPathName);
    }
    else
    {
    ...
    // Construct and return the complete path for the dataset directory.
    printf("\n\n\n\n==========\n create_directory dataDirectoryPathName: %s", dataDirectoryPathName);
   
    
    //char *dataSetDirectory = combine_char_ptr("/", dataSetFileName);
    //dataSetDirectory = combine_char_ptr(dataSetDirectory, directoryName);
    //dataSetDirectory = combine_char_ptr(dataDirectoryPathName, dataSetDirectory);
    //printf("\n\n\n\n==========\n create_directory dataSetDirectory: %s\n\n==========\n\n", dataSetDirectory);
 
    
    
    free(directoryPathName);
    free(dataSetFileName);
    //free(dataDirectoryPathName);
